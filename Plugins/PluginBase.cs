...
namespace Starburst.Plugins
{
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        private string GetAccessTokenManagedIdentity(string[] scopes)
        {
            ...
            try
            {
                this.TracingService.Trace("Scopes in GetAccessTokenManagedIdentity: " + string.Join(", ", scopes));
                // scope here is the resource URL :: example --> https://org9dfaa538.api.crm.dynamics.com/.default
                token = this.ManagedIdentityService.AcquireToken(scopes);
            }
            catch (Exception ex)
            {
                this.Trace($"Failed to acquire token {ex.Message}");
            }
